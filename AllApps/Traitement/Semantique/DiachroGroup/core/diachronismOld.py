...
class diachronism:
    def __init__(self, periode1, periode2, DiviAllML):
        ...
        self.Pt_s = self.PTsachantS["P1VersP2"]
        self.Ps_t = self.PTsachantS["P2VersP1"]
        ...
        self.ComputeLocalAverage(self.P1, self.PTsachantS["P1VersP2"], DiviAllML, self.P2)
        self.ComputeLocalAverage(self.P2, self.PTsachantS["P2VersP1"], DiviAllML, self.P1)
        ...
        self.MatchCluster(self.P1, self.P2, self.PTsachantS["P1VersP2"], self.PTsachantS["P2VersP1"], self.PA["P1"],
                          self.PA["P2"], self.A["P1"], self.A["P2"], self.D["P1"], self.D["P2"], )
    ...
    def ComputeProbTKnowingS(self, AllEnsSource, AllEnsCible, indic):
        ...
        print("\n")
        print("--- DETAIL COMPUTE PROBABILITE DE " + AllEnsCible["name"] + " SACHANT " + AllEnsSource[
            "name"] + "-------------")
        ...
        print("-- FIN DETAIL COMPUTE PROBABILITE DE " + AllEnsCible["name"] + " SACHANT " + AllEnsSource[
            "name"] + "-------------")

        print("\n")
        print("-- RESULTAT COMPUTE PROBABILITE DE " + AllEnsCible["name"] + " SACHANT " + AllEnsSource[
            "name"] + "-------------")

        print(self.PTsachantS[indic])

    def ComputeLocalAverage(self, Ens, ProbaResultEns, DiviAllML, Ens2):
        ...
        for elt in Ens["data"]:
            ...
            # ESSAI ATTENTION si n'est pas égal à valeur par défaut ("Non")
            else:
                print(SommePTsachantS)
                print(len(Ens2["data"]))
                ProbaActiviteS = SommePTsachantS / len(Ens2["data"])
                print(ProbaActiviteS)
                print("\n")
    ...
    def MatchCluster(self, AllEnsSource, AllEnsCible, AllProba2sachant1, AllProba1sachant2, PA1, PA2, A1, A2, D1, D2):
        ...
        for eltS in AllEnsSource["data"]:
            for eltT in AllEnsCible["data"]:
                ...
                Proba2sachant1List = [elt["proba"] for elt in AllProba2sachant1 if
                                      (elt["cible"] == eltT and elt["source"] == eltS)]
                Proba1sachant2List = [elt["proba"] for elt in AllProba1sachant2 if
                                      (elt["cible"] == eltS and elt["source"] == eltT)]
                ...
                # après l'aquation 15
                if Proba2sachant1 >= SpecificPA1 and (Proba2sachant1 >= A1 + D1) and Proba1sachant2 >= SpecificPA2 and (
                    Proba1sachant2 >= A2 + D2):
